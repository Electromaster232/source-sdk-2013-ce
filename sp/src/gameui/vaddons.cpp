...
{
	BaseClass::DrawDialogBackground( "ADD-ONS", NULL, "Desc", NULL );
}
...
void Addons::OnCommand(const char *command)
{
	...
	else if ( V_strcmp( command, "BrowseWorkshop" ) == 0 )
	{
		if ( steamapicontext->SteamFriends() )
			steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage("http://steamcommunity.com/app/799900/workshop/");
	}
	else
	{
...
{/*
	...
	hlUInt VPKPackage;
	if ( !hlCreatePackage( HL_PACKAGE_VPK, &VPKPackage ) )
	{
		Msg("Error creating package for addon vpk");
		hlShutdown();
		return;
	}
	hlBindPackage( VPKPackage );
	if ( !hlPackageOpenFile( szAddonVPKFullPath, HL_MODE_READ | HL_MODE_QUICK_FILEMAPPING | HL_MODE_VOLATILE ) )
	{
		Msg("Error opening addon vpk package");
		hlShutdown();
		return;
	}
	
	// Construct the output path for the addoninfo.txt and write it out
	V_snprintf( szAddonInfoFullPath, sizeof( szAddonInfoFullPath ), "%s%s", szModPath, ADDONS_DIRNAME );
	HLDirectoryItem *pItem = hlFolderGetItemByPath(hlPackageGetRoot(), ADDONINFO_FILENAME, HLFindType(HL_FIND_CASE_SENSITIVE | HL_FIND_FILES));
	if ( pItem )
	{
		hlItemExtract(pItem, szAddonInfoFullPath);
	}

	// Construct the output path for the addonimage.jpg and write it out
	V_snprintf( szAddonInfoFullPath, sizeof( szAddonInfoFullPath ), "%s%s", szModPath, ADDONS_DIRNAME );
	pItem = hlFolderGetItemByPath(hlPackageGetRoot(), ADDONJPEG_FILENAME, HLFindType(HL_FIND_CASE_SENSITIVE | HL_FIND_FILES));
	if ( pItem )
	{
		hlItemExtract(pItem, szAddonInfoFullPath);
	}
